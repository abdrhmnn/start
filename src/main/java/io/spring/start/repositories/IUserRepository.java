...
public interface IUserRepository extends JpaRepository<User, Integer> {
  ...
  // @Query(value = "SELECT new io.spring.start.dto.User(u.username) FROM User u
  // WHERE u.role.id=:role_id")

  // public List<io.spring.start.dto.User>
  // getUsernameAndPassword(@Param("role_id") Integer role_id);

  // Native Query SQL
  // @Query(value = "SELECT * FROM tb_m_user WHERE role_id = :role_id",
  // nativeQuery = true)
  // public List<User> getAllUserWithNativeQuery(@Param("role_id") Integer
  // role_id);
}
